...
- Monorepo signals: multiple `pyproject.toml` files, a `[tool.uv.workspace]` / Poetry path-dependency layout, or a populated `packages/*` (or `libs/*`) each with its own package. These are present only in a genuinely large multi-package repo; their absence means single-context, which is almost every repo.
